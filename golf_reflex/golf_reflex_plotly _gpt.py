...
def build_echarts_option(df: pd.DataFrame, selected_player: str):
    if df.empty:
        return {}
    ...
    for _, r in df.iterrows():
        brutto_data.append({
            "value": int(r["Brutto"]) if pd.notna(r["Brutto"]) else None,
            "date": r["Datum"].strftime("%d.%m.%y"),
            "turnier": str(r["Turnier"]),
            "club": str(r["Club"]),
            "player": str(r["Spieler_Name"]),
            "modus": str(r.get("Spielmodus", "")),
            "par": str(r.get("Par", "")),
            "cr": str(r.get("CR", "")),
            "brutto": int(r["Brutto"]) if pd.notna(r["Brutto"]) else "-"
        })
    ...
    # --- LINE DATA (HCP) ---
    if selected_player != "Alle Spieler":
        ...
        "tooltip": {
            ...
            function (params) {
                ...
                return `
                    ...
                    ${d.turnier}<br/>
                    ${d.club}<br/>
                    <b>Spieler:</b> ${d.player}<br/>
                    <b>Modus:</b> ${d.modus}<br/>
                    <b>Brutto:</b> ${d.brutto}<br/>
                    <b>Par:</b> ${d.par}<br/>
                    <b>CR:</b> ${d.cr}
                `;
            }
...
class GolfState(rx.State):
    ...
    def echarts_option(self) -> dict:
            ...
            df = self.filtered_df.copy().reset_index(drop=True)
            return build_echarts_option(df, self.selected_player)
# --- UI COMPONENTS ---
def chart_controls():
    return rx.cond(
